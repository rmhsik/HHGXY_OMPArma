...
int main(){
    ...
    for (int i=0; i<p.Nt;i++){
        for(int j=0;j<p.Nz;j++){
            ...
            Mx_dl.col(j) = -std::complex<double>(0.0,1.0)*Hx.col(0)*dt/4.0;
            Mx_d.col(j)  = 1-std::complex<double>(0.0,1.0)*Hx.col(1)*dt/4.0;
            Mx_du.col(j) = -std::complex<double>(0.0,1.0)*Hx.col(2)*dt/4.0;

            Mpx_dl.col(j) = std::complex<double>(0.0,1.0)*Hx.col(0)*dt/4.0;
            Mpx_d.col(j)  = 1+std::complex<double>(0.0,1.0)*Hx.col(1)*dt/4.0;
            Mpx_du.col(j) = std::complex<double>(0.0,1.0)*Hx.col(2)*dt/4.0;
        }
        for(int j=0;j<p.Nx;j++){
            ...
            Mz_d.col(j)  = 1+std::complex<double>(0.0,1.0)*Hz.col(1)*dt/4.0;
            Mz_du.col(j) = std::complex<double>(0.0,1.0)*Hz.col(2)*dt/4.0;

            Mpz_dl.col(j) = -std::complex<double>(0.0,1.0)*Hz.col(0)*dt/4.0;
            Mpz_d.col(j)  = 1-std::complex<double>(0.0,1.0)*Hz.col(1)*dt/4.0;
        ...
        StepX(Mx_dl,Mx_d,Mx_du,Mpx_dl,Mpx_d,Mpx_du,PsiX,Psi,p.Nx,p.Nz);
        Psi = Psi%Mask;
